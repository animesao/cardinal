# Copyright 2024-2026 dck maintainers
...
SRC_URI="https://github.com/animesao/dck/archive/v${PV}.tar.gz -> ${P}.tar.gz"

LICENSE="MIT"
SLOT="0"
KEYWORDS="~amd64 ~arm64"

# dck's runtime test suite needs:
...
# Empty, dck has no install-time deps other than what `go module` pulled
...
src_prepare() {
    ...
    # the resulting binary's `-X dck/cmd.version` stamping will still
...
src_compile() {
    ...
    #   * -X dck/cmd.version=${PV}  stamps the embedded version var
    CGO_ENABLED=0 \
    GOCACHE="${T}/go-cache" \
    go build -trimpath \
...
src_install() {
    ...
    # autostart can run `dck bootstrap` from inside the installed binary.

    newinitd "${FILESDIR}/dck.initd" dck 2>/dev/null || true
    newconfd "${FILESDIR}/dck.confd" dck 2>/dev/null || true
}

pkg_postinst() {
    einfo "dck installed. Run \`dck doctor\` to verify"
